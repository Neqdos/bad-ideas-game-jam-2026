...
@export var overide_screen_size: Vector2
...
func _interpolate_position(target_position: Vector2, delta: float) -> void:
	...
	if _follow_axis_is_locked:
		match follow_axis_lock:
			FollowLockAxis.X:
				locked_target_position.y = target_position.y
			FollowLockAxis.Y:
				locked_target_position.y = target_position.x
	else:
		locked_target_position = target_position
	
	global_position = locked_target_position
...
func _camera_frame_rect() -> Rect2:
	var screen_size: Vector2 = overide_screen_size if overide_screen_size else _phantom_camera_manager.screen_size
